...
namespace MultiComboBox.CustomCombo
{
    /// <summary>
    ...
    public partial class CheckBoxComboBox : PopupComboBox
    {
        ...
        public void SetStatusList(StatusList statusList)
        {
            ...
            var statusSelections = new ListSelectionWrapper<Status>(statusList, "Name");
            DataSource = statusSelections;
            DisplayMemberSingleItem = "Name";
            DisplayMember = "NameConcatenated";
            ValueMember = "Selected";
        }

        public void SetSelected(List<int> selectedList)
        {
            if (_statusList == null)
            {
                return;
            ...
            for (var i = 0; i < _CheckBoxComboBoxListControl.Items.Count; i++)
            {
                var item = _CheckBoxComboBoxListControl.Items[i];
                var status = _statusList[i];
                item.Checked = selectedList.Contains(status.Id);
            }
        }

        public List<int> GetSelectedKeyList()
        {
            if (_statusList == null)
            {
                return new List<int>();
